...
from Components import DText

class CommandHandler:
    
    def get_command(self, command_number, workfiles, destination_directory, destination_file, elements):
        try:
            if destination_directory == None: Error().show_error('No valid destination directory entered.')
            ...
            for element in elements:
                if isinstance(element, DText): element.clearContent()
            
        except ValueError as e:
            pass

    def merge(self, workfiles, destination_directory, destination_file):
        if len(workfiles) < 2: Error().show_error("Enter at least two file to merge.")
